...
def parse_gemini_files(content: str) -> list[tuple[str, str]]:
    """Parse ===FILE: path=== ... ===END_FILE=== blocks from Gemini output.

    Returns list of (filepath, file_content) tuples with stray delimiters
    stripped. This is the function that was broken — it used to leave
    trailing '===' in the output, corrupting every file the Bug Worker wrote.

    The fix: use ===END_FILE=== as the split boundary to extract content,
    then strip any remaining === artifacts.
    """
    import re
    files = []

    # Match ===FILE: <path>=== <content> ===END_FILE===
    pattern = re.compile(
        r'===FILE:\s*(.+?)===\s*\n(.*?)===END_FILE===',
        re.DOTALL,
    )
    ...
    for match in pattern.finditer(content):
        ...
        # Final safety: strip any stray === lines
        lines = file_content.split("\n")
...
class BugWorker:
    ...
    async def process_next_bug(self) -> Optional[dict]:
        ...
        try:
            from persistence.db import get_open_bugs_sorted, update_bug, reset_stuck_bugs

            # Reset any bugs stuck in IN_PROGRESS with errors (retry after 1 hour)
            reset_count = reset_stuck_bugs(stuck_minutes=60)
            if reset_count > 0:
                logger.info("bug_worker_reset_stuck", count=reset_count)
    ...
    async def _generate_fix(self, bug: dict) -> dict:
        ...
        try:
            ...
            # Parse files from response using fixed parser
            ...
            if files_created:
                ...
                return {"success": False, "error": "No valid files generated by Gemini"}
